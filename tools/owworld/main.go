...
// factionOf maps a map id to its campaign faction by name prefix.
func factionOf(m string) string {
	switch {
	case strings.HasPrefix(m, "wiz"):
		return "wizard"
	case strings.HasPrefix(m, "war"):
		return "warrior"
	case strings.HasPrefix(m, "con"):
		return "conjurer"
	}
	return ""
}
...
func main() {
	...
	for _, m := range include {
		...
		fmt.Fprintf(&b, "local FACTION = %q -- this zone's campaign faction\n\n", factionOf(m))
...
// the shared gate engine appended to every zone script. FACTION (the map's
// campaign: wizard/warrior/conjurer) is written just above this per map.
